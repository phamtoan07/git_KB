-- Start of DDL Script for Package Body FOTEST.CSPKS_FO_ACCOUNT_DEV
-- Generated 16-Oct-2018 10:17:37 from FOTEST@FO

CREATE OR REPLACE 
PACKAGE cspks_fo_account_dev AS
...
CREATE OR REPLACE 
PACKAGE BODY cspks_fo_account_dev AS
       ...
       /* --Khong hien thi lenh goc tren man hinh chi tiet khop lenh: tuning mstrade 25/aug/2016
         ...
         */
         SELECT ord.ORIGINORDERID ROOTORDERID, ord.ORDERID|| substr(t.TRADEID,-10,10) ORDERID, 'Y' ISDETAIL,
                 ord.TXDATE, TO_CHAR(t.TIME_EXEC,'HH24:MI:SS') TXTIME,
                ' ' AS ORSTATUSVALUE,
                0 QUOTEPRICE,
                0 ORDERQTTY,
                0 REMAINQTTY,
...
IF p_pageno IS NOT NULL THEN
      OPEN p_recordset FOR
       SELECT rn, TXDATE, CUSTODYCD , ACCTNO , SYMBOL ,  TIME_SEND,
              ...
              FOACCTNO, ORSTATUSVALUE,CUSTID, HOSESESSION
       FROM (
         ...
         (
             ...
             SELECT TXDATE, CUSTODYCD , ACCTNO , SYMBOL , to_char(time_send,'hh24:mi:ss') TIME_SEND,
               ...
               SELECT t.TXDATE, t.CUSTODYCD, t.ACCTNO, t.SYMBOL, t.TIME_SEND,
                  ...
                  t.ISDISPOSAL, t.REMAINAMT, t.SEQNUM, t.LASTCHANGE, t.TRADEPLACE, t.FOACCTNO, t.ORSTATUSVALUE, t.CUSTID, t.HOSESESSION
                FROM (
                  SELECT
                   ...
                   (CASE WHEN ord.NORB <> 'B' AND q.via <> 'L' AND (((ord.SUBSTATUS = 'SS'  OR ord.SUBSTATUS = 'ES')
                    ...
                    CASE WHEN instr.BOARD='UPCOM' AND ord.SESSIONEX = 'CNT' THEN 'O' ELSE '' END END END END END HOSESESSION
                  ...
                  WHERE ord.CUSTODYCD = c.CUSTODYCD
                    ...
                    AND ord.SUBTYPECD LIKE v_exectype
                    AND ord.SUBSTATUS <> 'MM'
                    AND ord.SUBSIDE NOT IN ('CB','CS')
                    AND ord.SYMBOL LIKE v_symbol
                   -- AND (NVL(p_isadvanced,'N') ='N'
                   --       OR (p_isadvanced ='Y' AND q.classcd ='DLO' AND ord.SUBSTATUS = 'SS' AND ord.remain_qtty >0)
                   --      )
         ...
         UNION ALL
            select v_today txdate, a.CUSTODYCD, a.ACCTNO, q.SYMBOL, q.TIME_SEND TIME_SEND,
                ...
                (CASE WHEN length(q.userid) > 4 THEN q.userid ELSE '' END) CUSTID, ' ' HOSESESSION
      ...
      OPEN p_recordset FOR
       SELECT TXDATE, CUSTODYCD , ACCTNO , SYMBOL , to_char(time_send,'hh24:mi:ss') TIME_SEND,
           ...
           SELECT t.TXDATE, t.CUSTODYCD, t.ACCTNO, t.SYMBOL, t.TIME_SEND,
              ...
              NVL(t.CUSTID,v_defaultTLID) CUSTID, t.HOSESESSION
            FROM(
   ...
   IF f_symbol IS NOT NULL THEN
    BEGIN
        SELECT  (nvl(p.TRADE,0) - nvl(p.SELLINGQTTY,0) - nvl(px.SELLINGQTTY,0)) TRADE,
                (nvl(p.BOD_RT3,0) + nvl(px.BOD_RT3,0) + nvl(p.BOD_RT1,0) + nvl(p.BOD_RT2,0)) RECEIVING
...
-- End of DDL Script for Package Body FOTEST.CSPKS_FO_ACCOUNT_DEV
