...
plog.setbeginsection (pkgctx, 'pr_txlog');
   ...
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
      VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'60',TO_NUMBER(p_txmsg.txfields('60').value),NULL,'Is credit dcroutamt');   plog.debug(pkgctx,'Check if neccessary to poplulate FEETRAN and VATTRAN');
...
BEGIN
 ...
 --<<BEGIN OF PROCESS SEMAST>>
    l_acctno := p_txmsg.txfields('35').value;
    SELECT count(*) INTO l_count
    FROM SEMAST
    WHERE ACCTNO= l_acctno;

    IF l_count = 0 THEN
         l_afacctno := substr(p_txmsg.txfields('35').value,1,8);
         l_codeid := substr(p_txmsg.txfields('35').value,length(p_txmsg.txfields('35').value)-1, length(p_txmsg.txfields('35').value));
         BEGIN
             SELECT b.setype,a.custid
             INTO l_sectype,l_custid
             FROM AFMAST A, aftype B
    ...
    BEGIN
         SELECT A.FULLNAME CUSTNAME, A.ADDRESS, A.IDCODE LICENSE, A.CUSTODYCD,B.BANKACCTNO,l_acctno
         INTO p_txmsg.txPrintInfo('35').custname,p_txmsg.txPrintInfo('35').address,p_txmsg.txPrintInfo('35').license,p_txmsg.txPrintInfo('35').custody,p_txmsg.txPrintInfo('35').bankac,p_txmsg.txPrintInfo('35').value
         FROM CFMAST A, AFMAST  B
         WHERE A.CUSTID = B.CUSTID
         AND EXISTS (
             SELECT 1 FROM SEMAST
    ...
    --<<END OF PROCESS SEMAST>>
--<<BEGIN OF PROCESS SEMAST>>
    l_acctno := p_txmsg.txfields('34').value;
    SELECT count(*) INTO l_count
    FROM SEMAST
    WHERE ACCTNO= l_acctno;

    IF l_count = 0 THEN
         l_afacctno := substr(p_txmsg.txfields('34').value,1,8);
         l_codeid := substr(p_txmsg.txfields('34').value,length(p_txmsg.txfields('34').value)-1, length(p_txmsg.txfields('34').value));
         BEGIN
             SELECT b.setype,a.custid
             INTO l_sectype,l_custid
             FROM AFMAST A, aftype B
    ...
    BEGIN
         SELECT A.FULLNAME CUSTNAME, A.ADDRESS, A.IDCODE LICENSE, A.CUSTODYCD,B.BANKACCTNO,l_acctno
         INTO p_txmsg.txPrintInfo('34').custname,p_txmsg.txPrintInfo('34').address,p_txmsg.txPrintInfo('34').license,p_txmsg.txPrintInfo('34').custody,p_txmsg.txPrintInfo('34').bankac,p_txmsg.txPrintInfo('34').value
         FROM CFMAST A, AFMAST  B
         WHERE A.CUSTID = B.CUSTID
         AND EXISTS (
             SELECT 1 FROM SEMAST
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('08').value,'SEMAST','08','3380')<>'TRUE' then
         ...
         plog.setendsection (pkgctx, 'C_SA_TLTX_NOT_ALLOW_BY_ACCTNO');
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('35').value,'SEMAST','35','3380')<>'TRUE' then
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('34').value,'SEMAST','34','3380')<>'TRUE' then
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
         plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
         RETURN errnums.C_BIZ_RULE_INVALID;
     End if;
     ...
     IF NOT ( INSTR('ANBC',l_STATUS) > 0) THEN

        p_err_code := '-900019';
        plog.setendsection (pkgctx, '900019');
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
     ...
     IF NOT ( INSTR('ANBTC',l_STATUS) > 0) THEN
        p_err_code := '-400100';
        plog.setendsection (pkgctx, '400100');
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;


     l_SEMASTcheck_arr := txpks_check.fn_SEMASTcheck(p_txmsg.txfields('35').value,'SEMAST','ACCTNO');

     l_STATUS := l_SEMASTcheck_arr(0).STATUS;

     IF NOT ( INSTR('ANBPC',l_STATUS) > 0) THEN
        p_err_code := '-900019';
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
     ...
     l_SEMASTcheck_arr := txpks_check.fn_SEMASTcheck(p_txmsg.txfields('34').value,'SEMAST','ACCTNO');
...
plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
     ...
     IF NOT ( INSTR('AVM',l_STATUS) > 0) THEN
        p_err_code := '-300014';
        plog.setendsection (pkgctx, '300014');
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      ...
      /*INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0019',ROUND(p_txmsg.txfields('13').value,0),NULL,p_txmsg.txfields ('01').value,p_txmsg.deltd,p_txmsg.txfields ('01').value,seq_SETRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');
*/
...
/*
      UPDATE SEMAST
         SET
           NETTING = NETTING + (ROUND(p_txmsg.txfields('13').value,0)), LAST_CHANGE = SYSTIMESTAMP
   ...
   ELSE -- Reversal
    ...
    UPDATE CIMAST SET LASTDATE=TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),
           RECEIVING=RECEIVING - (ROUND(p_txmsg.txfields('10').value,0)), LAST_CHANGE = SYSTIMESTAMP
    WHERE ACCTNO=p_txmsg.txfields('03').value;
...
BEGIN
   ...
   LOOP
       plog.debug(pkgctx, 'Bein delete: ' || p_txnum  || '-' || p_txdate) ;
       FETCH pv_refcursor
       INTO l_tllog;
       ...
       loop
       begin
           plog.debug(pkgctx, 'field:' || rec.FLDCD || '-' || rec.CVALUE);
       ...
       plog.debug(pkgctx, 'Begin revert');
